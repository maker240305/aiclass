# Environment Settings & Strict Rules
...
1. Run `chcp 65001` to set UTF-8 for the session.
...
- **Prefer English names** for Excel sheets/ranges (e.g., 'Scan').
...
1. **IF** file is `.reg`, `.bat`, `.cmd`, `.csv`:
   → **MUST use `encoding='cp949'`** (EUC-KR).
...
* **Notification:** Keep user informed of task status and any progress.

## Language & Communication
* **Thinking & Code Logs:** English.
* **User-facing Output:** Korean (한국어).
